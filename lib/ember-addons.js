...
const NPM_SEARCH_URL = 'http://npmsearch.com/query?q=ember-addon&size=8000';

/**
 * Get names of all ember related npm packages.
 */
async function getAll() {
  let response = await request({ url: NPM_SEARCH_URL, json: true });
  let addons = response.body.results.reduce((filteredAddons, row) => {
    let name = row.name[0];
